...
export const kasusMR01Operations = {
  ...
  convertFormDataToTableData(formData: Record<string, any>, baseData: Partial<KasusMR01>): KasusMR01 {
    ...
    return {
      ...
      pasien_nama: getField(formData, 'pasien_nama', 'Pasien_nama', 'nama_pasien', 'Nama_pasien'),
      pasien_nik: getField(formData, 'pasien_nik', 'Pasien_nik', 'nik_pasien', 'NIK_pasien'),
      pasien_tgl_lahir: this.formatDateForDB(getField(formData, 'pasien_tgl_lahir', 'Pasien_tgl_lahir', 'tgl_lahir_pasien', 'Tanggal_lahir_pasien')),
      pasien_umur: getField(formData, 'pasien_umur', 'Pasien_umur', 'umur_pasien', 'Umur_pasien') ? Number(getField(formData, 'pasien_umur', 'Pasien_umur', 'umur_pasien', 'Umur_pasien')) : null,
      pasien_jk: getField(formData, 'pasien_jk', 'Pasien_jk', 'jenis_kelamin', 'Jenis_kelamin'),
      pasien_alamat: getField(formData, 'pasien_alamat', 'Pasien_alamat', 'alamat_pasien', 'Alamat_pasien'),
      pasien_rt_rw: getField(formData, 'pasien_rt_rw', 'Pasien_rt_rw'),
      pasien_kelurahan: getField(formData, 'pasien_kelurahan', 'Pasien_kelurahan'),
      pasien_kecamatan: getField(formData, 'pasien_kecamatan', 'Pasien_kecamatan'),
      ...
      gejala_ruam: getField(formData, 'gejala_ruam', 'Gejala_ruam', 'ruam', 'Ruam'),
      ...
      gejala_mata_merah: getField(formData, 'gejala_mata_merah', 'Gejala_mata_merah'),
      gejala_lain: getField(formData, 'gejala_lain', 'Gejala_lain'),

      sedang_dirawat: getField(formData, 'sedang_dirawat', 'Sedang_dirawat'),
      rumah_sakit: getField(formData, 'rumah_sakit', 'Rumah_sakit'),
      tanggal_dirawat: this.formatDateForDB(getField(formData, 'tanggal_dirawat', 'Tanggal_dirawat')),
      ...
      kontak_kasus_lain: getField(formData, 'kontak_kasus_lain', 'Kontak_kasus_lain'),
      bepergian_2_minggu: getField(formData, 'bepergian_2_minggu', 'Bepergian_2_minggu'),
      tempat_bepergian: getField(formData, 'tempat_bepergian', 'Tempat_bepergian'),
      tanggal_bepergian: this.formatDateForDB(getField(formData, 'tanggal_bepergian', 'Tanggal_bepergian')),
      sumber_infeksi: getField(formData, 'sumber_infeksi', 'Sumber_infeksi'),

      spesimen_diambil: getField(formData, 'spesimen_diambil', 'Spesimen_diambil'),
      jenis_spesimen: getField(formData, 'jenis_spesimen', 'Jenis_spesimen'),
      tanggal_pengambilan: this.formatDateForDB(getField(formData, 'tanggal_pengambilan', 'Tanggal_pengambilan')),
      ...
      kondisi_akhir: getField(formData, 'kondisi_akhir', 'Kondisi_akhir'),
      ...
      jumlah_kontak: getField(formData, 'jumlah_kontak', 'Jumlah_kontak') ? Number(getField(formData, 'jumlah_kontak', 'Jumlah_kontak')) : null,
